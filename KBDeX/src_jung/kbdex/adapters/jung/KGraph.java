...
public class KGraph<V, E> extends AbstractGraph<V, E> implements Graph<V, E> {
	/**
	 ...
	 */
	public KGraph() {
		//optimization 2011/4/21
		vertex_maps = new LinkedHashMap<V, Map<V, E>[]>();
		directed_edges = new FastHashMap<E, Pair<V>>();
		undirected_edges = new FastHashMap<E, Pair<V>>();
		//vertex_maps = new FastHashMap<V, Map<V, E>[]>();
		//vertex_maps = new LinkedHashMap<V, Map<V, E>[]>();
	...
	public Collection<E> getInEdges(V vertex) {
		...
		Collection<E> in = new HashSet<E>(vertex_maps.get(vertex)[INCOMING]
				.values());
	...
	public Collection<E> getOutEdges(V vertex) {
		...
		Collection<E> out = new HashSet<E>(vertex_maps.get(vertex)[OUTGOING]
				.values());
	...
	public Collection<V> getPredecessors(V vertex) {
		...
		Collection<V> preds = new HashSet<V>(vertex_maps.get(vertex)[INCOMING]
				.keySet());
	...
	public Collection<V> getSuccessors(V vertex) {
		...
		Collection<V> succs = new HashSet<V>(vertex_maps.get(vertex)[OUTGOING]
				.keySet());
	...
	public List<V> getValidVertices() {
		List<V> valids = new ArrayList<V>();
		Collection<V> vertices = getVertices();
		for (V v : vertices) {
			if ((v instanceof KBElement) && ((KBElement) v).isValid()) {
				valids.add(v);
			}
		}
		return valids;
